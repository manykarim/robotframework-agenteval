...
- **DF-3.3-S5 (DOGFOOD-FINDING-pabot-not-verified, LOW)** — AC-3.3.9 requires `pabot --processes 4` parallel execution with no orphan-process leak. Story 3.3 verified single-process execution (15/15 pass); pabot multi-process verification was NOT performed due to time budget. The Story 3.1 cleanup contract (try/except BaseException → stack.aclose()) is the load-bearing primitive; theoretical correctness holds. Phase-1.5 hygiene: run `uv run pabot --processes 4 tests/dogfood/rf-mcp/test_mcp_surface_parity.robot` + assert post-suite `pgrep -fc robotmcp.server == 0`.
