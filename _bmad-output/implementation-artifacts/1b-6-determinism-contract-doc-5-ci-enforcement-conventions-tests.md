...
**And** `tests/unit/conventions/test_error_class_hierarchy.py` walks `src/AgentEval/errors.py` + asserts every exported error class in `__all__` inherits from `AgentEvalError` — dynamically exempting any class that inherits from `Warning` (covers Story 1b.2's `DegradedTraceWarning` per architecture L997 H_R4 fix + Epic 11 Story 11.3's future `AdapterVersionDriftWarning` per contract L83, per Story 1b.6 code-review Codex MED catch). No orphan error classes — every leaf goes through one of the 4 ratified sub-bases (`AgentEvalIntegrityError`, `AgentEvalBudgetError`, `AgentEvalCompatError`, `AgentEvalSafetyError`) per ADR-014. End-of-Epic-1b state: all 8 currently-implemented leaves (`IncompleteTraceError`, `PollingDisallowedError` + `TierViolationError` shipped by Story 1b.6's H3 patch, `CostExceededError`, `RuntimeBudgetExceededError`, `AdapterDiscoveryError`, `DuplicateRegistrationError`, `UnsupportedBinaryVersionError`) verified — test passes on current `errors.py` baseline.
...
- `uv run pytest tests/unit -q` — 263 prior unit + 7 new conventions tests + 10 violation-detection tests from H6 Blind#12 patch = 280+ pass.
...
**`src/AgentEval/` changes** — Story 1b.6 code-review H3 patch added 2 leaves to `errors.py` (`PollingDisallowedError` + `TierViolationError`) for the determinism-contract.md §(c) reference to be load-bearing (per Auditor F7 + Codex H4 + Blind#7 catch). The 2 leaves had been forward-referenced in `errors.py` L66+L68 since Story 1b.2; raise sites still land in Epic 6 per `docs/contracts/error-class-hierarchy.md` L89+L91. Apart from the 2-leaf addition, conventions tests are test infra (per architecture CI-Enforcement Pattern L1127-1138, NOT the pre-edit "L853 test-infra-not-sub-library" citation per Auditor F8 catch — L853 is the Cross-Sub-library Import Discipline section, not the test-infra rule).

## Change Log

| Date       | Version | Description                                                                  | Author |
| ---------- | ------- | ---------------------------------------------------------------------------- | ------ |
| 2026-05-19 | 0.3.0   | Cross-LLM code-review patches applied; status → done. 4-reviewer pair (Blind + Edge + Auditor + Codex 0.117.0 gpt-5.4): 69 raw → ~25 unique after dedup → 6 HIGH + 5 MED + 10 LOW/cleanup patches applied + 6 deferred to Phase-1.5 + ~10 dismissed. **12th consecutive cross-LLM STAR catch** (Codex unique): (a) `Stat.Run N Times` return type drift — contract said `list[AgentRunResult]` but PRD L1534 ratifies `list[KeywordRun]`; (b) `Stat.Get Pass At K` return shape drift — contract said `(passed, total, rate)` tuple but PRD L1535 ratifies `float ∈ [0,1]`. **Auditor STAR**: PRD L1211 single-paragraph determinism summary missing entirely from contract doc (FR63 directly violated); duplicated Tier Model section (9 dead lines); `PollingDisallowedError` + `TierViolationError` documented as raising but the leaves were never added to errors.py — Story 1b.6 H3 patch ships both class declarations alongside the contract publication (Epic 6 owns the raise sites per error-class-hierarchy.md L89+L91 IMPLEMENTED markers). Patches applied: H1 PRD L1211 summary verbatim added to contract; H2 dead Tier Model duplicate deleted; H3 PollingDisallowedError + TierViolationError added to errors.py + contract L89/L91 IMPLEMENTED markers; H4 FR26/FR27 return-type drift fixed in contract §(b); H5 badge-text drift fixed in spec D3 + AC-1b.6.6; H6 _walk.py drops `*Library` suffix heuristic + uses fully-qualified module name + classmethod/staticmethod unwrap + SRC_ROOT assertion (Codex+Edge+Blind cluster); test_error_class_hierarchy dynamic Warning detection (Codex MED); test_violation_detection.py NEW (10 tests) proves convention tests CATCH violations per Blind#12 fake-green-prevention. 6 deferred to Phase-1.5 (deferred-work.md DF-1b.6-S1..S6). All-gates clean: ruff/format/mypy/license (32 source files now — `src/AgentEval/errors.py` extended; up from 31); 280 unit (263 prior + 17 conventions — 7 + 10 new violation-detection); 30 conformance + 11 skipped + 6 tier1 + RF smoke regression. Epic 1b CLOSED. | Amelia |
