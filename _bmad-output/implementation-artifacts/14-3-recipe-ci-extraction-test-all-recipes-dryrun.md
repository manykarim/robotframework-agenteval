...
- **C64 (DF-8b.3-S1)** in `docs/phase-1-5-carry-overs.md` L91: harness implemented against this catalog row. ⚠️ **PARTIAL** (mechanism delivered; full closure tracked under DF-14.3-S1).
...
- **L-1 (Story 14.2 Codex HIGH-A → Story 14.3 verification)**: re-derive every citation from source. Story 14.3's spec citations to Epic 11 retro L157 + Epic 12 retro L168 + Epic 13 retro L186 + C64 L91 verified via direct grep before writing.
...
- [x] **Task 3: Helper-function unit tests (AC-14.3.5)** — DONE. **14 helper/unit tests** (≥5 required; cross-LLM MED-2 corrected the earlier "10" count — the v2 code patch added the unclosed-block + nested-fence + eligible/passing-floor tests): regex extraction (empty-md returns []; counts match grep across all recipes); classify_block (full suite / test-cases-only / settings-only / fragment → 4 tests); wrap_block (settings-only raises ValueError; fragment raises ValueError; test-cases-only prepends Library import; full suite unchanged); collect-passable threshold sanity.
...
- [x] **Task 5: All-gates pass + Story 14.2 hook integration (AC-14.3.9)** — DONE. `uv run pytest tests/` → **1981 passed + 32 skipped + 5 warnings** (+17 vs 1964 Story 14.2 baseline; the harness adds 13 parametrized tests + 2 negative + 10 helper = +25 new; net +17 because some parametrizations SKIP-pass instead of testing). `uv run python scripts/check-catalog-references.py --all-tracked` → EXIT 0 ✓ (Story 14.2 hook passes — DF-14.3-S1 catalogued in deferred-work.md). `uv run ruff check src/ tests/` → "All checks passed!" ✓. `uv run mypy src/` → "Success: no issues found" ✓.
...
- Catalog: `docs/phase-1-5-carry-overs.md` L91 (C64 / DF-8b.3-S1).
...
- **AC-14.3.5**: 14 helper/unit tests (≥5 required; cross-LLM MED-2 count correction).
...
- **L-1 (Story 14.2 Codex HIGH-A → 14.3)**: re-derive citations from source. Story 14.3 spec citations verified pre-write via grep against `epic-11-retro` L157 + `epic-12-retro` L168 + `epic-13-retro` L186 + `C64` L91.
...
**New files:**
- `tests/integration/recipes/test_all_recipes_dryrun.py` — 430+ LoC harness with FencedRobotBlock + helpers + 20 parametrized block cases + 2 negative regression-guards + 14 helper/unit tests = **36 collected (20 passed / 16 skipped)** (cross-LLM MED-2 count correction).
