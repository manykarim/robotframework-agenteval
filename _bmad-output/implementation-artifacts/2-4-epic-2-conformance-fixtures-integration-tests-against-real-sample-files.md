...
So that AC-CONFORMANCE-01 is **extended** for Epic 2 (PRD's AC-CONFORMANCE-01 letter scopes adapter fidelity oracles per PRD L522-533 — covered by Story 1b.5 + Epic 4 adapter conformance tests; this story EXTENDS the spirit by adding a parallel static-inspection fixture surface for Tier-1 file-parsing keywords per Story 2.4 D-B carve-out), the SIMPLICITY-02 keyword-idiom conformance test un-skips, and regressions are caught by CI before merge. Auditor HIGH 2026-05-19 amendment: pre-edit story Goal claimed "AC-CONFORMANCE-01 is satisfied" — corrected to "extended" per the load-bearing adapter-oracle-vs-Tier-1-fixture distinction.
...
| 2026-05-19 | 0.3.0   | Code-review patches applied. 4-reviewer pair (Blind+Edge-cases+Auditor+Codex 0.117.0): 16th consecutive cross-LLM STAR catch. 3-way HIGH (Blind+Auditor+Codex): ci.yml comment claimed `pytest tests/conformance -q` was an existing step; it was NOT. Story 2.4's 22 conformance cases + SIMPLICITY-02 introspections were UNGATED on PR. Added the explicit step. Blind+Codex 2-way HIGH: `expected_result_shape` schema was open object so typos like `contains_key` silently disabled assertions. Strict-schema'd with enum + additionalProperties:false. Auditor unique HIGH: Story Goal claimed "AC-CONFORMANCE-01 is satisfied" — actually PRD AC-CONFORMANCE-01 scopes adapter fidelity oracles only; Story 2.4 EXTENDS the spirit via the static-inspection carve-out. Goal sentence + Dev Notes amended. Edge-cases MED: fixture path-traversal vulnerability (input_file_path could escape REPO_ROOT) — added path-resolution guard. Edge-cases MED: test_dogfood_prep_epic_3_unblocked was fake-green (`len >= 1` trivially true); tightened to assert both rf-mcp servers + FR5 entry shape. Blind MED: 6 error fixtures missing `expected_field_name_substring`; added to all 5 + 1. Blind MED: `_assert_result_shape` accepted any list (empty included); extended with `min_length` + `contains_values` + tightened `skill_get_allowed_tools_happy` fixture + `skill_get_disable_model_invocation_happy` with `equals: false`. Blind LOW: no `server_name`-scoped MCP fixtures; added 2 (happy + scoped-unknown-server error). Auditor MED: AC-SIMPLICITY-02b paired-getter rule wasn't enforced; added `test_core_ergonomic_should_keywords_have_paired_getters` (trivially passes Phase-1; load-bearing for Epic 4+). All-gates green: ruff/format/mypy clean (39 src files); 541 unit+conformance+tier1+integration combined + 10 skipped + 13 RF. Status: done. | Dev |
