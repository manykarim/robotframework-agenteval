...
- **FR3:** Agent Surface Author can call `Subagent.Get Frontmatter <path.md>` against `.claude/agents/*.md` files and receive the parsed sub-agent definition; same matcher surface as `Skill.Get Frontmatter`.
...
- **FR5:** Agent Surface Author can call `MCP.Get Server Config <.mcp.json path>` and receive a dict of declared MCP servers (`name`, `command`, `args`, `env`, `transport`) without starting any of them.
