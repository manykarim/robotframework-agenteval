...
- **FR4:** Agent Surface Author can call `Hook.Get Config <settings.json path>` and receive a dict mapping `hooks.<event>` → list of hook entries (`command`, `args`, `timeout`, `matcher`); supports `PreToolUse`, `PostToolUse`, `Stop` events and inline-skill-frontmatter hooks.
  - **DRIFT RESOLVED 2026-07-08 (change `accept-real-claude-hook-config`, design D6):** This FR4 description documented an *assumed* flat hook shape that empirically diverges from the real Claude Code `settings.json` schema (nested matcher groups of typed hook definitions; the flat form was rejected by the real tool — findings dossier E2). Per `feedback_spec_vs_ratified_doc_precheck` ("fix-the-losing-source-NOW"), the losing source is this PRD. The **ratified hook schema is now `openspec/specs/hook-config-parsing/spec.md`**: `Get Config` accepts the real nested format (primary) + legacy flat (deprecated alias), returns entries keyed by PLAIN event name (`config["PreToolUse"]`, replacing the former flattened `config["hooks.PreToolUse"]`), with `type` always present. The FR4 text above is retained verbatim for historical traceability; the spec supersedes it.
...
- **FR22:** Agent Developer can call `Metric.Get Token Usage <run>` returning `Usage(input_tokens, output_tokens, cached_input_tokens)` per Story 1b.2 ratified dataclass at `src/AgentEval/types.py:132` (PRD amendment 2026-05-20 per Story 6.1 code-review Auditor HIGH-A drift fix — pre-edit `Usage(input, output, total)` was the pre-1b.2 draft; the ratified shape has no `total` field; consumers compute `total = input_tokens + output_tokens` at the call site when needed); `Metric.Get Latency <run>` returning mean turn-level latency in ms; `Metric.Get Latency P95 <run>` returning P95 latency; `Metric.Get Cost Total <run>` returning USD cost.
...
- **FR55 (cohort heatmap format):** `Metric.Get Cohort Heatmap <DiscoverabilityResult>` (and equivalent for any multi-cohort metric, including `DiscoverabilityComparisonResult` per Story 13.3) returns a `CohortHeatmap` object with `as_ascii() -> str` (default terminal output: 2-decimal Pass@k floats with task rows × model columns — the ratified orientation per Story 8b.2 implementation + Story 13.4 Codex HIGH-2 fix-the-losing-source-NOW 2026-06-01; pre-edit said "model rows × task-cluster columns" + "✓/✗/•" but the shipped surface emits floats not glyphs and orients tasks as rows for legibility under wide model cohorts), `as_html() -> str` (Phase 2 — Story 13.4 ships this; standalone HTML5 document with embedded CSS color-coded by Pass@k), `as_dict() -> dict` (machine-readable). `write_html(path: str | Path) -> Path` is a thin file-write convenience companion per epic L2203 + Story 13.4 D-2; not a new contract surface. Verifiable via fixture matching the documented ASCII format. Story 13.3 D-1 + Opus LOW-1 fix-the-losing-source-NOW 2026-06-01: stale `ToolDiscoverabilityResult` type name → `DiscoverabilityResult` (the FR10a-shipped type).
...
- **FR61:** Library MUST emit a `DegradedTraceWarning` (Python `Warning` subclass declared in `AgentEval.errors`) whenever a recoverable trace-quality degradation event occurs mid-run (e.g., hosted-MCP observer connection drop, JSONL backend write failure, RunManifest sidecar emit failure, span missing required `agenteval.*` attributes) WITHOUT raising — test outcomes are not masked (per AC-5.3.2). If the degradation invalidates server-side MCP observation coverage, the affected run's `mcp_coverage` falls to `"external_mixed"` per the ADR-016 degradation rules (the 3-state Literal `{"hosted_in_process", "subprocess_with_observer", "external_mixed"}` admits NO `"partial"` value — superseded 2026-05-17 ADR-016 ratification). Verifiable via fixture intentionally triggering each degradation class.
- **FR62:** `Get Last Warnings    test_id=<value>` returns the list of warnings emitted during the run as a list of `WarningRecord` dataclass instances. Each record carries 5 fields: `warning_type` (str — the Python warning class name, e.g., `"DegradedTraceWarning"`, `"AdapterVersionDriftWarning"`), `message` (str — the human-readable warning text), `source` (str — the emitting subsystem identifier, e.g., `"telemetry.listener"`, `"mcp.observer"`, `"telemetry.run_manifest"`), `timestamp` (datetime — UTC RFC 3339, set at `_kernel/warnings.record_warning()` call time — Story 5.4 code-review Auditor MED-5 fix 2026-05-20: pre-edit said "set at `warnings.warn()` call time" but the structured channel + Python channel are now ordered such that `record_warning` fires first per Story 5.4 HIGH-C fix, and the dataclass timestamp is bound to the structured-channel call), `remediation` (str | None — actionable advice for the test author; None when no remediation is known). Accepts `test_id="current"` (current bound test scope), `test_id="<id>"` (specific test), or `test_id="all"` (whole-suite). Verifiable via fixture intentionally triggering each warning class.
