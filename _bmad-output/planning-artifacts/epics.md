...
**Source-of-truth ratification (2026-05-18):** Story 1b.2 pre-create-story drift check (6th consecutive use) caught 3 pre-emptive drifts in Story 1b.4 spec vs ratified sources: (D7) `__agenteval_tier__` dunder vs `_agenteval_tier` single-underscore per architecture L620 + Story 1b.1's actual implementation (dunder rejected by L1 review-finding fix); (D4) `AgentRunResult` location `src/AgentEval/coding_agent/types.py` vs top-level `src/AgentEval/types.py` per architecture L853 ("Cross-sub-library data flow goes through ... shared types in `agenteval/types.py`"); (D8) `mcp_coverage` Literal includes `"none"` (4-state) vs ratified ADR-016 3-state value space (NO `"none"`). All 3 resolved by honoring ratified sources; epics.md updated 2026-05-18 ahead of Story 1b.4 create-story.

**Given** the kernel modules from Stories 1b.1-1b.3,
**When** I implement `src/AgentEval/coding_agent/base.py`,
**Then** the module exposes a `CodingAgentAdapter` Protocol with required methods: `send_prompt(prompt: str, **kwargs) -> AgentRunResult`, `run_scenario(scenario: Scenario, **kwargs) -> AgentRunResult`, `connect_mcp_server(name: str, server: MCPServer) -> None`, `disconnect_mcp_server(name: str) -> None`, `name: str` property, `version: str` property, `_agenteval_tier: int = 3` class attribute (single-underscore convention per architecture L620; Story 1b.1's `tier.py` decorator sets the same attribute name on individual keywords).
...
**And** `AgentRunResult` dataclass is **added to `src/AgentEval/types.py`** (the top-level types module co-created by Story 1b.2 per architecture L853, NOT to a sub-library types.py) with fields: `response_text: str`, `tool_calls: list[ToolCall]`, `completeness: Literal["complete", "truncated", "partial"]` (per PRD FR36a L1553 — D-completeness drift caught by Story 1b.2 code-review pre-emptive sweep; epics.md L963 previously used `"full"/"partial"/"incomplete"` which contradicted PRD ratified vocabulary), `mcp_coverage: Literal["hosted_in_process", "subprocess_with_observer", "external_mixed"]` (per FR36b + ratified ADR-016 — **3-state value space; NO `"none"` value** per ADR-016 §Decision L24-28), `token_usage: TokenUsage`, `cost_usd: float`, `latency_seconds: float`, `trace_id: str`.
