...
**Given** the ratified ADRs from Epic 0 — ADR-016 (was ADR-A6) amended with Story 0.1 trust-floor + adapter contract findings; ADR-018 (was ADR-A8) ratified as-is with Story 0.2 cross-cutting confirmation; ADR-004 (was ADR-007) amended with Story 0.1 handler-wrap pattern,
**When** I implement `src/AgentEval/_kernel/context.py`,
**Then** the module exposes a `TestContext` dataclass containing `test_id` (str), `suite_id` (str), `mcp_per_test_mode` (Literal["suite", "test", "process"]), and a context-var-backed `current_context()` accessor + `bind_context(ctx)` / `unbind_context()` lifecycle functions. Cleanup semantics match the spike findings document (Story 0.2 output) — concrete behavior cited in the module docstring.

**And** `src/AgentEval/_kernel/tier.py` exposes a `@tier(N: Literal[1, 2, 3])` decorator that attaches `__agenteval_tier__` attribute to the decorated function, plus a `get_keyword_tier(func) -> int | None` accessor, plus a `tier_badge(tier: int) -> str` helper returning the libdoc badge text (e.g., `"[Tier 1 — Deterministic]"`).

**And** `src/AgentEval/_kernel/run_async.py` exposes `_run_async(coro)` per ADR-A1 — runs the coroutine via `asyncio.run()` from a synchronous context, with worker-thread fallback when called from an already-running event loop (IDE runners, nested test executions); no `nest_asyncio` import by default.

**And** unit tests in `tests/unit/kernel/test_{context, tier, run_async}.py` cover: `context.py` — bind/unbind round-trip, ContextVar isolation across threads, cleanup semantics for each mode; `tier.py` — decorator attaches attribute, libdoc badge text matches expectation, invalid tier raises ValueError; `run_async.py` — sync-context invocation, nested-loop fallback path, exception propagation.
...
**Then** the module exposes a `TraceStore` class wrapping OTel SDK `InMemorySpanExporter` (per architecture Step-4 Decision-2) with per-test-scope partitioning (uses `current_context().test_id` to key spans); supports `add_span(span)`, `get_spans(test_id: str | None = None) -> list[Span]`, `clear(test_id: str | None = None)`; spans are returned in chronological order; cross-test isolation is enforced (test A's `get_spans()` never returns test B's spans).

**And** `src/AgentEval/_kernel/redaction.py` exposes a `redact(text: str, patterns: list[Pattern] | None = None) -> str` function that scrubs known credential patterns (API keys matching `sk-*`, `ANTHROPIC_API_KEY=*`, OpenAI keys, bearer tokens) from arbitrary text + a `redact_dict(d: dict) -> dict` recursive variant for nested config objects. Default pattern list ships in the module; opt-in extension via `register_pattern(regex: str)`.

**And** `src/AgentEval/_kernel/coverage.py` exposes `compute_mcp_coverage(run_data: RunData) -> Literal["hosted_in_process", "subprocess_with_observer", "external_mixed"]` per AC-MCP-OBSERVE-01 + ADR-016 (was ADR-A6, ratified 2026-05-17 with D1 trust-floor semantics — strongest complete path wins; see `docs/adr/ADR-016-mcp-coverage-detection-default.md`); the function inspects the trace store + RunData metadata to determine which observation path provided the data. **The 3-state field is the ratified value space — NOT 4-state. There is no `"none"` value; runs with no observed traces report `external_mixed` per ADR-016's catch-all safe default.**

**And** unit tests in `tests/unit/kernel/test_{trace_store, redaction, coverage}.py` cover: `trace_store.py` — per-test isolation, chronological ordering, clear semantics; `redaction.py` — known patterns scrubbed, plain text unchanged, nested dict recursion, custom pattern registration; `coverage.py` — each of the **3 enum values** (`hosted_in_process` / `subprocess_with_observer` / `external_mixed` per ratified ADR-016) produced from representative trace fixtures + at least one explicit trust-floor test (both paths fire → reports stronger path).
...
**Given** the Python entry-points group `robotframework_agenteval.adapters` from Story 1a.1's pyproject.toml,
**When** I implement `src/AgentEval/_kernel/discovery.py`,
**Then** the module exposes `discover_adapters() -> dict[str, type[CodingAgentAdapter]]` (loads all registered entry-points via `importlib.metadata.entry_points`), `register_adapter(name: str, cls: type)` (programmatic registration per FR17b), `get_adapter(name: str) -> type` (lookup with `UnknownAdapterError` raised on miss). Entry-point loading errors are caught + logged but do not crash discovery (one broken third-party adapter cannot block library import).

**And** `src/AgentEval/_kernel/guardrails.py` exposes `@guarded_fanout(cost_kwarg: str = "max_cost_usd", runtime_kwarg: str = "max_runtime_seconds")` per ADR-A5 — a decorator that wraps any fan-out keyword (signature `(self, *args, max_cost_usd: float = 5.00, max_runtime_seconds: float | None = None, **kwargs)`) with: (a) pre-flight cost estimate via `_estimate_cost(args, kwargs)` hook, raising `CostExceededError` if projected > `max_cost_usd`; (b) wall-clock meter raising `RuntimeBudgetExceededError` at 1.1× `max_runtime_seconds` if set; (c) per-trial accumulation so partial-execution costs are surfaced in exception messages.

**And** unit tests in `tests/unit/kernel/test_{discovery, guardrails}.py` cover: `discovery.py` — entry-point loading, programmatic registration, lookup miss, broken-entry-point graceful degradation; `guardrails.py` — cost pre-flight raise, runtime meter raise at 1.1×, cost accumulation across trials, partial-result exception message format.
...
**When** I implement `src/AgentEval/coding_agent/base.py`,
**Then** the module exposes a `CodingAgentAdapter` Protocol with required methods: `send_prompt(prompt: str, **kwargs) -> AgentRunResult`, `run_scenario(scenario: Scenario, **kwargs) -> AgentRunResult`, `connect_mcp_server(name: str, server: MCPServer) -> None`, `disconnect_mcp_server(name: str) -> None`, `name: str` property, `version: str` property, `__agenteval_tier__: int = 3` class attribute.

**And** the module exposes `InProcessAdapter` (abstract base inheriting from Protocol) with default implementations of MCP server lifecycle using in-process MCP SDK connections, plus an abstract `_invoke_llm(prompt: str, **kwargs) -> RawResponse` method subclasses implement.

**And** the module exposes `SubprocessAdapter` (abstract base inheriting from Protocol) with default implementations of MCP server lifecycle using subprocess MCP transport, plus abstract `_subprocess_command() -> list[str]` and `_parse_stream_output(output: str) -> AgentRunResult` methods subclasses implement, plus binary-version-check helper `_assert_binary_version(binary: str, min: str, max: str | None)` raising `UnsupportedBinaryVersionError` per FR47.

**And** `AgentRunResult` dataclass is defined in `src/AgentEval/coding_agent/types.py` with fields: `response_text: str`, `tool_calls: list[ToolCall]`, `completeness: Literal["full", "partial", "incomplete"]` (per FR36a), `mcp_coverage: Literal["hosted_in_process", "subprocess_with_observer", "external_mixed", "none"]` (per FR36b), `token_usage: TokenUsage`, `cost_usd: float`, `latency_seconds: float`, `trace_id: str`.

**And** unit tests in `tests/unit/coding_agent/test_base.py` cover: Protocol structural typing checks (mypy verifies a stub subclass conforms), ABC instantiation rejected without abstract method implementations, `_assert_binary_version` raises correct error type, default lifecycle methods exercise the kernel `context.py` properly.
...
**Given** the JSON+jsonschema format choice from architecture Step-4 Decision-4,
**When** I author `tests/conformance/fixture-schema.json`,
**Then** the schema validates fixture shape: required fields `id`, `tier` (1|2|3), `description`, `keyword_under_test`, `inputs` (dict), `expected_outputs` (dict), `expected_errors` (list, optional), `oracle_type` (literal: `"exact_match"`, `"contains"`, `"trajectory_match"`, `"completeness_check"`).

**And** `tests/conformance/loader.py` exposes `load_fixtures(path: Path) -> list[Fixture]` validating each loaded file against `fixture-schema.json` (raises `InvalidConformanceFixtureError` with JSON Pointer on schema violation).

**And** `tests/conformance/harness.py` exposes a `run_fixture(fixture: Fixture, library: AgentEval) -> ConformanceResult` function executing the fixture against the live library, returning structured pass/fail with oracle-specific evidence (exact match diff / containment witness / trajectory similarity score / completeness flag).

**And** 6 reference fixtures land at `tests/conformance/fixtures/`: (a) `fix-skill-frontmatter-valid.json` (Tier-1, FR1, exact_match), (b) `fix-skill-frontmatter-invalid-yaml.json` (Tier-1, FR1, expected_errors → InvalidSkillFrontmatterError), (c) `fix-mcp-tool-schema-valid.json` (Tier-1, FR6, exact_match), (d) `fix-mcp-list-tools-roundtrip.json` (Tier-2, FR9a, trajectory_match), (e) `fix-trace-completeness-truncated.json` (Tier-3, FR36a + AC-CONFORMANCE-02, completeness_check), (f) `fix-tool-discoverability-mvp-single-runtime.json` (Tier-3, FR10a MVP, contains).

**And** the `conformance.yml` CI workflow from Story 1a.2 is wired to invoke the harness across all fixtures and report per-fixture pass/fail; the trivial PR check from Story 1a.2 now exercises these 6 fixtures (all pass against the still-empty library because each fixture is gated on its target keyword existing — fixtures whose target keyword isn't yet implemented surface as `SKIP` with reason, not `FAIL`).
...
As **Mei (Agent Surface Author — MCP author mode)** or **Priya (QA Engineer)**,
I want `MCP.Start Server`, `MCP.Connect`, `MCP.Stop Server` keywords supporting all 3 transports (stdio, streamable_http, in-memory), with MCP spec version validated at connect time and per-test scope cleanup honored under `pabot`,
...
**And Given** the same lifecycle calls under `pabot --processes 8` with `mcp_per_test="test"` mode,
...
**And** the keywords carry `[Tier 2 — LLM-Deterministic]` libdoc badges (per Story 1b.1's tier.py — Tier 2 because the server runtime is deterministic but I/O introduces latency variance); conventions tests from Story 1b.6 all pass on the new keywords.
...
**Then** `providers/base.py` exposes an `LLMProviderAdapter` Protocol with methods: `complete(prompt: str, model: str, **kwargs) -> ProviderResponse`, `name: str` property. `litellm_adapter.py` implements the Protocol via `litellm.completion()` (sync wrapper around async via `_run_async`). `mock.py` provides deterministic stub responses for unit tests. `factory.py` resolves a provider by name from the entry-points group `robotframework_agenteval.providers` or programmatic registration.

**And Given** the `InProcessAdapter` ABC from Story 1b.4,
**When** I implement `src/AgentEval/coding_agent/generic.py` (`GenericAdapter(InProcessAdapter)`),
**Then** the adapter accepts `provider: str = "litellm"`, `model: str` (e.g., "anthropic/claude-sonnet-4-6", "openai/gpt-4o", "ollama/llama3"), and additional provider-specific kwargs; calling `adapter.send_prompt("Hello")` returns an `AgentRunResult` with `response_text` populated, `token_usage` extracted from the provider response, `cost_usd` computed via `litellm.completion_cost()`, `latency_seconds` measured, `tool_calls=[]` (Send Prompt has no tool surface), `completeness="full"`, `mcp_coverage="none"`.

**And Given** the Generic adapter and a model that returns tool calls (e.g., Anthropic Claude with tool use enabled),
**When** I call `adapter.send_prompt("Use the search tool to find X")` with the adapter pre-connected to an MCP server providing a `search` tool,
**Then** the returned `AgentRunResult` has `tool_calls` populated with each tool invocation captured from the provider's response, `mcp_coverage="hosted_in_process"` (per Story 1b.2's `compute_mcp_coverage`), and the OTel spans for the tool execution are captured in the trace store from Story 1b.2.

**And** the Generic adapter is registered via the `robotframework_agenteval.adapters` entry-points group as `"generic"`; `discover_adapters()` from Story 1b.3 returns `{"generic": GenericAdapter, ...}` after Story 4.1 lands.
...
**Given** the `SubprocessAdapter` ABC from Story 1b.4 and a Claude Code installation (`claude` binary on `$PATH`, version within the agenteval-pinned compat range — e.g., `>=1.5.0,<2.0.0`),
**When** I implement `src/AgentEval/coding_agent/claude_code_cli.py` (`ClaudeCodeCLIAdapter(SubprocessAdapter)`),
**Then** the adapter overrides `_subprocess_command()` to return `["claude", "--output-format=stream-json", ...]` with the prompt fed via stdin, and `_parse_stream_output(output)` to deserialize the stream-json into `AgentRunResult` (response_text from final assistant turn, tool_calls from `tool_use` content blocks, token_usage from final usage, cost_usd from `costUSD` field or computed, latency_seconds from start/end timestamps, completeness derived from stream-json terminal event, mcp_coverage="external_mixed" by default — full coverage detection per Epic 5).

**And Given** a `claude` binary version outside the pinned compat range (e.g., version `0.9.0` below the floor),
**When** the adapter is instantiated and `_assert_binary_version()` runs,
...
**And Given** the adapter and a connected MCP server (via `connect_mcp_server` from the SubprocessAdapter base),
**When** `adapter.send_prompt("Use the X tool")` runs,
**Then** the `claude` subprocess is invoked with the MCP server registered in its `.mcp.json` (a temporary `.mcp.json` is generated for the subprocess session and cleaned up on exit per per-test scope from Story 1b.1), tool calls executed by `claude` are captured via the stream-json output, and `AgentRunResult.mcp_coverage="external_mixed"` reflects that observation happens via stream-json parsing (not in-process span capture — Epic 5's hosted-MCP observer changes this when applicable).

**And** the adapter is registered via the `robotframework_agenteval.adapters` entry-points group as `"claude-code-cli"`; declared as `[claude-code]` optional extra in `pyproject.toml` (`uv sync --extra claude-code` installs the adapter's deps + a doc note about needing the `claude` CLI on `$PATH`).
...
**And Given** a scenario YAML file at `tests/fixtures/scenarios/multi-turn-search.yaml` (with multiple turns, tool expectations, response constraints),
...
**Then** the scenario YAML is loaded + validated against a schema at `src/AgentEval/scenarios/schema.py` (raises `InvalidScenarioYAMLError` with line + JSON Pointer on schema violation), the scenario is executed turn-by-turn against the adapter with the named MCP servers attached, and the returned `AgentRunResult` captures the multi-turn conversation + all tool calls.
...
**And Given** the configuration precedence chain from FR41 (defaults < library kwargs < environment < scenario YAML < per-keyword kwargs),
**When** I call `${config}=    Get Effective Config` in a `.robot` test after setting values at each precedence level,
**Then** the returned dict shows the resolved value for each setting plus a `_provenance` sub-dict naming the precedence level that "won" for each setting; calling `Get Effective Config    setting=max_cost_usd` returns just that setting's resolved value + provenance.
...
**And** all assertions integrate with AssertionEngine for the operator surface (`==`, `!=`, `contains`, `matches`); unit tests cover all 4 trajectory modes + 3 response assertion variants + IncompleteTraceError propagation.
