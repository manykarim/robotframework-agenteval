...
- [ ] 2.1 Add per-item format classification in `parse_hook_config`: matcher
...
- [ ] 2.2 Implement matcher-group handling: validate group `matcher` is str
...
- [ ] 2.3 Implement per-type definition validation: `type` non-empty str when
      ...
      (design D3)
- [ ] 2.4 Normalize legacy flat entries through the existing command-entry
...
- [ ] 2.5 Change return keys from `f"hooks.{event}"` to plain `event` (design
      D2, BREAKING)
- [ ] 2.6 Extend `inline_skill` extraction + reserved-key rejection to
      real-format command definitions with 5-segment pointers; keep unknown
      entry fields passing through onto returned entries
- [ ] 2.7 Rewrite every fix suggestion that teaches the flat shape to quote the
      real nested schema (design D4); update the module docstring's format
      description
- [ ] 2.8 Run 5-nullish-variant checks on new required fields (`type`, `url`,
...
- [ ] 3.1 Update the `Get Config` docstring: real nested input schema as
...
- [ ] 3.2 Update the module docstring usage block (`${config["hooks.PreToolUse"]}`
      → `${config}[PreToolUse]`)
- [ ] 3.3 Smoke-verify docstring examples via the existing docstring-example
...
- [ ] 4.1 Migrate `tests/unit/hooks/test_library.py` off `"hooks.<event>"`
      keys; update pointer assertions to nested pointers where fixtures moved
- [ ] 4.2 Add unit tests for every spec scenario: real-format parse, matcher
...
- [ ] 4.3 Migrate `tests/unit/hooks/test_robot_integration.robot` to
      `${config}[PreToolUse]` access
- [ ] 4.4 Migrate conformance consumers:
...
- [ ] 4.5 Migrate `tests/integration/static_inspection/test_real_world_samples.py`
      + add a case parsing `settings-real-world.json` asserting all events and
      non-command types survive normalization
- [ ] 4.6 Full gate: `uv run pytest tests/` green; `uv run ruff check src/ tests/`;
      `uv run mypy src/`

## 5. Docs + drift closure

- [ ] 5.1 Update the README `Get Config` snippet to the new access shape (ONLY
      the hooks snippet — README-wide fixes belong to `fix-first-run-experience`)
- [ ] 5.2 Annotate PRD FR4 drift per design D6: note in
...
- [ ] 5.3 Grep new/changed files for `DF-X-SY` markers and verify each is in
      `docs/phase-1-5-carry-overs.md` (per `feedback_carry_over_catalog_gate`,
      BEFORE review)
- [ ] 5.4 Note in `openspec/changes/add-hooks-execution-testing/` (when drafted)
