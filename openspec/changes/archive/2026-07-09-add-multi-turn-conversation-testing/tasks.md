...
- [x] 1.1 Add `ConversationTurn` (index, role, content, result, continuation)
...
- [x] 1.2 Add `ConversationClosedError` and
...
- [x] 1.3 Define `ConversationState` (prior turns + adapter-opaque
...
- [x] 2.1 Create `src/AgentEval/conversation/library.py` with
...
- [x] 2.2 Implement `Send Message` (Tier-2): append user turn → thread via
...
- [x] 2.3 Implement `Get Conversation Transcript` (frozen snapshot; aggregates
      reconcile with per-turn results) and `End Conversation` (close + release
      native resources; transcript stays readable)
- [x] 2.4 Implement `Transcript Should Contain` (Tier-1; role filter +
      `as_regex`; failure message reports text, role, turns inspected)
- [x] 2.5 Implement the shared transcript renderer (used by replay preamble,
      judge prompts, and simulator prompts — design Open Question 3 resolution)
- [x] 2.6 Compose `ConversationLibrary` into `_SUB_LIBRARIES` in
...
- [x] 2.7 Unit tests: lifecycle happy path on mock provider (deterministic, no
...
- [x] 3.1 Implement `GenericAdapter.run_turn()` building the full
      `messages=[...]` history from `conversation_state` (extends the
      single-message construction at `generic.py:216`)
- [x] 3.2 Unit tests: mock provider receives full history on turn 2+;
      turn 1 `continuation="initial"`, turn 2 `"native_session"`

## 4. Judge at turn and transcript level

- [x] 4.1 Document per-turn usage in `Judge.Get Score` docstring (works today —
...
- [x] 4.2 Implement `Judge Turn Should Pass` (Tier-2, `@guarded_fanout`;
...
- [x] 4.3 Unit tests with a stubbed judge adapter: turn selection, transcript
      rendering content, pass/fail assertion behavior

## 5. Conversational metrics

- [x] 5.1 Implement `Get Conversation Results` and `Get Turn Count` (Tier-1;
      accept handle OR transcript) in `src/AgentEval/metrics/library.py`
- [x] 5.2 Unit tests: extraction order, existing `Get Cost Total` /
      `Get Latency P95` aggregation over extracted lists
- [x] 5.3 Browser-Library-style docstrings with tier annotations + a chained
...
- [x] 6.2 Implement `ClaudeCodeCLIAdapter.run_turn()`: capture session id from
...
- [x] 6.3 Fixture-driven unit tests + gated live integration smoke
      (`AGENTEVAL_INTEGRATION_TESTS`, mirroring `tests/integration/test_*_live.py`)
- [x] 6.4 Add documented follow-up markers (carry-over convention) for native
...
- [x] 7.1 Implement the simulator core in
...
- [x] 7.4 Unit tests on mock provider: turn cap, sentinel stop + stripping,
...
- [x] 8.1 Extend `ScenarioEval` in `src/AgentEval/scenarios/schema.py` with
...
- [x] 8.2 Extend `Run Scenario` in `src/AgentEval/orchestration/library.py`:
...
- [x] 8.3 Unit tests: exactly-one-of validation matrix (both / neither / empty
...
- [x] 8.4 Update `docs/` scenario schema reference + release notes flagging the
      pre-1.0 breaking validation change

## 9. Docs, dogfood, and validation gates

- [x] 9.1 Recipe doc showing BOTH styles side by side (scripted `Send Message`
...
- [x] 9.2 Note in the recipe + design docs that `add-red-team-probes` builds
      multi-turn attack loops on `ConversationHandle`/`Simulate User` (do not
      implement attacks here)
- [x] 9.3 Dogfood `.robot` test under `tests/dogfood/` driving a scripted +
...
- [x] 9.4 Update README keyword tables + `docs/contracts/stability-surface.md`
      ...
      not worsen)
- [x] 9.5 Full gates: `uv run pytest tests/`, `uv run ruff check src/ tests/`,
