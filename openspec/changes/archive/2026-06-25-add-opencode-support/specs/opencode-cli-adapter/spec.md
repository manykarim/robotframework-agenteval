...
> **Amendment (apply-time, per `feedback_in_flight_spec_amendment` +
> `feedback_honest_framing`):** the original draft scenario claimed a
> *newer*-than-`_TESTED_UP_TO` binary emits the warning. That is factually wrong
> — the ratified `version_drift` helper fires only when the detected binary is
> *older* than tested (`tested.minor - detected.minor >= 2`) or on a previous
> major; newer-or-equal is an intentional silent no-op. Additionally, because
> this adapter's `MIN_VERSION` (`1.15.0`) and `_TESTED_UP_TO` (`1.15.12`) share
> the same minor, the within-range drift window is empty: any binary ≥2 minors
> behind tested is already below the floor and rejected by
> `_assert_binary_version` first. The helper is still wired (identical to
> codex/copilot) for when `_TESTED_UP_TO` later advances across a minor. The
> scenario is corrected to the honest, testable assertion.
...
> **Amendment (apply-time, per `feedback_in_flight_spec_amendment`):** the
> original draft required re-export from the `AgentEval.coding_agent` package
> `__init__`. The ratified codebase convention (6 existing adapters incl.
> `codex_cli`/`copilot_cli`) does NOT re-export concrete adapters from the
> package `__init__` — only the base classes/Protocol are re-exported, and
> concrete adapters are imported from their submodule. Re-exporting only
> `opencode-cli` would be inconsistent with its 6 siblings, so the requirement
> is amended to the submodule-import + entry-point convention
> (fix-the-losing-source: the draft spec was the losing source vs the ratified
> convention).
