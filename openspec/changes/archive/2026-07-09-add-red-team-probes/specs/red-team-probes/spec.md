...
Provide defensive, single-turn adversarial-robustness testing of a coding agent
the operator owns. The capability ships a curated, versioned probe pack across
four garak-style attack categories, keywords to run probes against any registered
adapter, refusal detection, and an attack-success-rate metric derived from the
existing Pass@k / Wilson fan-out machinery — with results integrating into the
cohort heatmap and cost-budget guardrails.
