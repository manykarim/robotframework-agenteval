...
- [x] 1.1 Probe a captured real Claude Code CLI trace (or capture one) to
...
- [x] 1.2 Create `src/AgentEval/subagents/types.py` with frozen dataclasses
      ...
      conventions
- [x] 1.3 Add the three error classes to `src/AgentEval/errors.py`:
...
- [x] 2.1 Create `src/AgentEval/subagents/_internal.py` with
...
- [x] 3.1 Implement `Subagent.Get Delegations` (`@tier(1)`) in
...
- [x] 3.2 Implement `Subagent.Should Have Delegated To` (`@tier(1)`) —
      case-sensitive exact match, `SubagentDelegationAssertionError` with
      expected/observed/fix_suggestion diagnostics on failure
- [x] 3.3 Implement `Subagent.Should Not Have Delegated` (`@tier(1)`) with the
      optional-subagent-name semantics (targeted vs any-delegation)
- [x] 3.4 Implement `Subagent.Get Routing Pass At K` (`@tier(1)`) delegating to
...
- [x] 4.1 Implement `Subagent.Should Delegate To` (`@tier(2)`) — single adapter
...
- [x] 4.2 Implement `Subagent.Get Delegation Decision` (`@tier(3)` +
      `@guarded_fanout()`) returning `DelegationDecision`; never raises on a
      routing miss; FR28 polling guard
- [x] 4.3 Implement `Subagent.Get Routing Accuracy` (`@tier(3)` +
...
- [x] 6.2 Unit tests for the three Tier-1 assertion/getter keywords covering
      every spec scenario (pass, fail-with-observed-listing, no-delegation,
      targeted absence)
- [x] 6.3 Unit tests for `Subagent.Get Routing Pass At K` — parity with
      `_compute_pass_at_k`, foreign-result-type counts as non-pass, ValueError
      paths
- [x] 6.4 Stub adapter fixture emitting synthetic Task `ToolCallTrace`s; unit
...
- [x] 6.5 Unit tests for `Get Routing Accuracy` — 2-task cohort scenario from
...
- [x] 7.2 Add the new keywords to the README keyword table + regenerate libdoc;
      run the libdoc-render smoke to confirm no `Subagent.`-name auto-split
      (multi-word post-dot norm)
- [x] 7.3 Full gate: `uv run pytest tests/`, `uv run ruff check src/ tests/`,
