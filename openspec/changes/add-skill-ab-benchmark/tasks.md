...
- [ ] 1.1 Add frozen dataclasses to `src/AgentEval/skills/types.py`:
...
- [ ] 1.2 Add `InvalidSkillBenchmarkTasksError` to `src/AgentEval/errors.py`
...
- [ ] 1.3 Implement `load_skill_benchmark_tasks` (benchmark cohort YAML
...
- [ ] 1.4 Unit tests for the loader covering: valid mixed-mode cohort, both
...
- [ ] 2.1 Implement the arm runner in `_benchmark.py`: per-arm N tasks ×
      ...
      latency
- [ ] 2.2 Implement deterministic `expected_content` grading (ALL substrings,
      case-insensitive, against `response_text`)
- [ ] 2.3 Implement blind judge grading: compose judge prompts from rubric +
...
- [ ] 2.4 Unit tests for the engine with the mock/generic adapter + canned
...
- [ ] 3.1 Wire Epic-13 primitives: Mann-Whitney U + Cliff's delta over
...
- [ ] 3.2 Implement the closed-set verdict rule (D6): `skill_improves` /
      `skill_regresses` / `skill_unnecessary` (baseline=none only, threshold
      + no-significant-improvement) / `no_significant_difference`
- [ ] 3.3 Assemble `SkillBenchmarkComparisonResult`: arm summaries
...
- [ ] 3.4 Unit tests: verdict truth table (all four verdicts + the
...
- [ ] 4.3 Unit tests for the keyword: validation matrix (polling, trials=0,
...
- [ ] 4.4 Verify conventions suites pass for the new keyword (verb allowlist,
...
- [ ] 5.1 Run `uv run pytest tests/`, `uv run ruff check src/ tests/`,
      `uv run mypy src/` — all green
- [ ] 5.2 Grep new files for `DF-X-SY` markers and register each in
...
- [ ] 5.3 Caller-count check on new public helpers
      (`feedback_caller_count_check`); document any 0-caller helper as a
      DF entry
- [ ] 5.4 Add the keyword to the README keyword table + docs/index.md counts
