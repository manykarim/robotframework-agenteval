...
- [ ] 2.2 `scenario.yaml`: rewrite `mcp_servers` as a list of strings; verify it loads via
      `Load Scenario` against `src/AgentEval/scenarios/loader.py`
- [ ] 2.3 Strip internal jargon from ALL files in `src/AgentEval/_init/templates/` (Story/Epic/
...
- [ ] 2.4 Manually verify in a clean temp dir: `agenteval init` then the scaffolded README's run
      command exits 0 with zero edits

## 3. CI scaffold smoke test

- [ ] 3.1 Add `tests/integration/test_init_scaffold_e2e.py`: subprocess `agenteval init` into
      `tmp_path`, run `robot` on the scaffolded suites (mock provider + bundled echo server),
      assert exit code 0; hard timeout
- [ ] 3.2 Same test module: `Load Scenario` the scaffolded `tests/fixtures/scenario.yaml` and
      assert it parses
- [ ] 3.3 Wire the smoke test into `ci.yml` (keyless job path) and confirm it runs on push; prove
...
- [ ] 4.1 Re-derive the true unique keyword count from libdoc across the 6 libraries; update
      `README.md` (currently "51 keywords / 6 libraries") and `docs/index.md` (currently
      "5 libraries · 49 keywords") to the derived number
- [ ] 4.2 Add the 6 missing keywords to the README tables: `Stat.Mann Whitney U`,
...
- [ ] 4.3 Add a docs-build CI check asserting the documented counts in `README.md` +
      `docs/index.md` match the libdoc-derived count (anchor on the canonical count sentence /
      table rows, not free prose)
- [ ] 4.4 Create `docs/troubleshooting/README.md` aggregating every per-recipe Symptom/Cause/Fix
      table, organized by symptom, each entry linking back to its recipe; add an entry for
      "scaffold from an older version fails"
- [ ] 4.5 Remove README links to `docs/coming-from/` and `docs/scenarios/` and delete the empty
...
- [ ] 4.6 Rewrite the README recipe table to describe recipes by what they demonstrate (removes
      the Devon/Raj/Many vs Priya/Mei persona mislabels as a side effect)
- [ ] 4.7 Fix the 4 recipe code blocks skip-listed in `_KNOWN_BROKEN_BLOCKS`
...
- [ ] 5.1 README "Writing a skill file" subsection: the 4 required frontmatter fields (`name`,
...
- [ ] 5.2 Document the hook config input schema `HooksLibrary.Get Config` accepts (flat
...
- [ ] 5.3 Write `docs/running-against-a-real-model.md`: provider selection
...
- [ ] 5.4 Add commented `ANTHROPIC_API_KEY=` / `OPENAI_API_KEY=` entries to `.env.example` with a
      pointer to the real-model page

## 6. Jargon strip + tier de-emphasis (README, index, recipes)

- [ ] 6.1 Strip Story/Epic/FR/ADR/DF-X-SY/C-number identifiers, persona-journey slots, and
...
- [ ] 6.2 Same pass over `docs/recipes/*.md` (prose and code comments), preserving the
      Symptom/Cause/Fix tables and `## Use case` / `## Keywords used` / `## Walkthrough` headings
      the docs-build check requires
- [ ] 6.3 Reduce the tier system in README/quick-start to one short note linking to the full
      tier documentation
- [ ] 6.4 Verify no regressions: grep the in-scope surfaces for identifier patterns (expect zero
      hits), run the docs-build checks and the recipe dryrun harness locally

## 7. Full verification

- [ ] 7.1 `uv run pytest tests/` green (including new smoke + Call Tool tests);
      `uv run ruff check src/ tests/` + `uv run mypy src/` clean
- [ ] 7.2 End-to-end replay of dossier E1 in a clean dir: `agenteval init` → documented run
      command → green, zero edits; `Load Scenario` on the scaffolded YAML succeeds
- [ ] 7.3 Confirm doc claims: README and `docs/index.md` counts match libdoc; README tables list
