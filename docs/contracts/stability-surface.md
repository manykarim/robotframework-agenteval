...
- `AgentEval.judge.library.JudgeLibrary.Get Score` (Story 12.1) — `experimental` label. Tier-2 LLM-judge keyword wired via `_SUB_LIBRARIES` standard composition path. Returns `JudgeScore` dataclass (`numeric_score` 0-10, `pass_threshold_met` vs rubric threshold, `reasoning`, `criteria_breakdown`, `cost_usd`). `@guarded_fanout` cost+runtime budgets forwarded from `AgentEval(max_cost_usd=..., max_runtime_seconds=...)` per ADR-015. Phase-1: Markdown rubric format only (YAML schema is DF-12.1-S1 / C79); no multi-turn chain-of-thought (Phase-2 work). Phase-1: NO retry loop on `JudgeOutputParseError` — seed+temperature=0 should make response deterministic. Class signature (`judge_adapter`, `judge_model`, `**adapter_kwargs`) is `experimental`; promotion to `stable` after the 3-month-no-break window per Epic 9 retro Action #3.
- `AgentEval.judge.types.JudgeScore` + `AgentEval.judge.types.JudgeRubric` (Story 12.1) — `experimental` label. Frozen dataclasses with strict `__post_init__` validation (`numeric_score` and `threshold` in `[0.0, 10.0]`; `cost_usd >= 0.0`; non-empty `criteria`). Defensive `dict()` copy of `criteria_breakdown` per Story 1b.2 M_R6 pattern. Promotion to `stable` after the same 3-month window.
- `AgentEval.judge.rubric.load_rubric` (Story 12.1) — `experimental` label. Markdown rubric parser; raises `InvalidJudgeRubricError` (FR59-shaped 5-line error) on missing sections / malformed bullets / unparseable threshold. Phase-1 Markdown-only; YAML schema is DF-12.1-S1 / C79.
- `AgentEval.judge.library.JudgeLibrary.Calibrate Rubric` (Story 12.2; keyword renamed from `Judge.Calibrate` to `Judge.Calibrate Rubric` post-libdoc-bug 2026-05-27 — single-word post-dot keyword names trigger DynamicCore's name auto-split in libdoc HTML). — `experimental` label. Tier-2 LLM-judge calibration workflow. Runs the judge against a YAML calibration set with human-labeled ground truth, computes Cohen's kappa over binarized judge-pass / human-pass labels at the rubric's threshold, sweeps thresholds for precision/recall/F1, and surfaces systematic-bias diagnostics. Returns `CalibrationReport` with `cohen_kappa`, `kappa_undefined_reason` (for zero-variance edge cases), `passes_hard_fail` (kappa ≥ 0.7 per `architecture.md` L199 agentguard borrow), `threshold_tuning` precision/recall/F1 sweep, `recommended_threshold` (F1-maximizing), `systematic_bias_diagnostics` bullets. Phase-1: Cohen's kappa only (Krippendorff's alpha is DF-12.2-S1 / C81); single-shot per row (multi-judge ensemble is DF-12.2-S1 / C81). Promotion to `stable` after the same 3-month window.
- `AgentEval.judge.types.CalibrationReport` (Story 12.2) — `experimental` label. Frozen dataclass with strict `__post_init__` validation (`rows_processed <= rows_total`; `judge_scores`/`human_labels` lengths match `rows_processed`; all `human_labels` in `[0.0, 10.0]`; `recommended_threshold` in `[0.0, 10.0]`; non-negative cost/latency). Defensive copy of `threshold_tuning` nested dict per Story 1b.2 M_R6 pattern.
- `AgentEval.judge.calibration.load_calibration_set` (Story 12.2) — `experimental` label. Strict YAML calibration set loader (`{rows: [{prompt, response, human_label}, ...]}`); raises `InvalidCalibrationSetError` (FR59-shaped 5-line error) on parse failure / missing required field / unknown extra key / nullish-input variant / `human_label` out of `[0.0, 10.0]`. Returns `tuple[CalibrationRow, ...]`.
- `AgentEval.judge.calibration.compute_cohen_kappa` (Story 12.2) — `experimental` label. Pure-Python Cohen's kappa over binarized judge-pass / human-pass labels at the configured `pass_threshold`. Returns `(kappa_value, undefined_reason)` tuple where `undefined_reason` is one of `"human_label_zero_variance"` / `"judge_score_zero_variance"` / `"expected_agreement_unity"` / `None`. Raises `ValueError` on length mismatch or empty sequences (programmer error — caller must validate first).
- `AgentEval.judge.calibration.KAPPA_HARD_FAIL_THRESHOLD` (Story 12.2) — `experimental` label. `Final[float] = 0.7` constant per `architecture.md` L199; `CalibrationReport.passes_hard_fail` is `True` iff `not isnan(kappa) and kappa >= KAPPA_HARD_FAIL_THRESHOLD`.
...
- `Stat.Mann Whitney U` RF keyword + Python method `StatsLibrary.compute_mann_whitney_u` — `provisional` label. Returns `MannWhitneyResult` (two-sided test only — `alternative="greater"`/`"less"` variants Phase-2 / DF-13.1-S1). Signature stable; `effect_size_r` computation is signed rank-biserial (`r = 2 * U1 / (n_a * n_b) - 1`) matching `Stat.Cliff Delta`'s sign convention. The returned `u_statistic` is `min(U1, U2)` (the canonical smaller-U form) — this normalizes the scipy default which returns `U1`; consumers needing scipy's `.statistic` recover U1 via the `effect_size_r` formula.
- `Stat.Cliff Delta` RF keyword + Python method `StatsLibrary.compute_cliff_delta` — `provisional` label. Returns scalar `float ∈ [-1.0, 1.0]` per PRD FR29b (NOT a dataclass — preserves AssertionEngine matcher compatibility per Story 6.3 D-1 precedent).
- `Stat.Bootstrap Confidence Interval` RF keyword + Python method `StatsLibrary.compute_bootstrap_ci` — `provisional` label. Returns `tuple[float, float]` (lo, hi) percentile bootstrap CI. Percentile method only (BCa + BC-corrected variants Phase-2 / DF-13.1-S2). Default `n_resamples=10_000` is `provisional`. `seed` parameter is REQUIRED (no default) per `@tier(1)` bit-identical guarantee (FR31a): omitting `seed` raises `ValueError`. Operators wanting OS-entropy randomness must pass `seed=random.randrange(2**32)` explicitly.
