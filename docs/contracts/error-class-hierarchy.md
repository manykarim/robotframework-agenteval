...
| Sub-base | Leaves |
| --- | --- |
| `AgentEvalSafetyError(AgentEvalError)` | `ValidateOperatorDisallowed` |
...
| `AdapterDiscoveryError` | `ADAPTER_DISCOVERY_ERROR` | `78` (EX_CONFIG) | Entry-points discovery encountered a partially-installed adapter package, an unknown-name lookup miss, OR a cross-package name collision (per ADR-013 L42+L43). The former `DuplicateRegistrationError` sub-leaf folded into this class by `remove-dead-machinery`; collision message names the colliding name + both sources. | **IMPLEMENTED — Story 1b.3 (`src/AgentEval/errors.py` leaf + `src/AgentEval/_kernel/discovery.{_discover_entry_point_group, _cached_coding_agents}` raise sites + `get_adapter` miss path).** |
...
```
INVALID_SKILL_FRONTMATTER: Skill `.md` frontmatter failed validation.
  File: tests/skills/broken_skill.md
  Line: 3
  Field: frontmatter.name
  Fix: Add a non-empty string `name:` field to the skill's YAML frontmatter block (per FR4 skill schema).
```
...
- **ADR-018**: `SandboxRequiredError` was planned as a sandbox-gate leaf but never shipped; the `security/` package was withdrawn pre-1.0 by `remove-dead-machinery` and the Protocol/error surface are re-introduced when a real Phase-3 sandbox backend lands.
