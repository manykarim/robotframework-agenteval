...
| `InvalidCalibrationSetError` | `INVALID_CALIBRATION_SET` | `65` (EX_DATAERR; same as other Tier-1 setup-failure errors) | A Judge calibration set YAML file passed to `Judge.Calibrate` has wrong extension OR fails YAML parse OR has non-dict top-level OR missing/malformed `rows:` list OR per-row missing required field (`prompt`/`response`/`human_label`) OR unknown extra key OR nullish-input variant (`None`/`""`/`False`) OR `human_label` out of `[0.0, 10.0]`. The `field_name` attribute carries the offending field path (e.g., `rows[3].human_label`). Format per FR59 + L96-104. | **Story 12.2** (Judge.Calibrate Phase-1 YAML calibration set loader); 24th ratified leaf. Phase-2 multi-judge ensemble / Krippendorff's alpha → DF-12.2-S1 / C81. |
