...
| `AgentEvalIntegrityError(AgentEvalError)` | `PollingDisallowedError`, `IncompleteTraceError`, `TierViolationError`, `InvalidSkillFrontmatterError`, `InvalidSubagentDefinitionError`, `InvalidHookConfigError` |

**Total: 14 leaves** (Safety: 2 + Budget: 2 + Compat: 4 + Integrity: 6). Story 2.1 pre-authoring amendment (2026-05-19) added `InvalidSkillFrontmatterError`; Story 2.2 pre-authoring amendment (2026-05-19) added `InvalidSubagentDefinitionError` + `InvalidHookConfigError` (both Tier-1 setup-failure semantics, parallel to `InvalidSkillFrontmatterError`). Adding additional leaves requires ADR amendment per ADR-014 §Decision.
...
- The **14 leaves' names + `error_code` values** are `stable`. Adding new leaves is minor-version-bump safe (consumers' `try/except AgentEvalError` still catches the new leaf via inheritance). Renaming a leaf or changing its `error_code` is a major-version-bump change (breaks consumers' specific catches).
