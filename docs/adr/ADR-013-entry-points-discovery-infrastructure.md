...
- Duplicate-name collisions across packages produce an `AdapterDiscoveryError` with the colliding entry-point name and both source package names; agenteval refuses to silently pick one. **Amendment (`remove-dead-machinery`, 2026-07-08):** the originally-ratified dedicated `DuplicateRegistrationError(AdapterDiscoveryError)` leaf was folded into `AdapterDiscoveryError` — it had a single raise site and no dedicated exit code (collisions already exited through `ADAPTER_DISCOVERY_ERROR`), so the leaf distinction bought nothing. The fail-closed behavior and the "both source package names" contract are unchanged (now carried in the message).
