...
- `Library AgentEval` composes all 11 sub-libraries (task 4.1 / 4.2).
...
# --------------------------------------------------------------------------- #
# Task 4.1 — single import reaches one keyword from each of the 11 sub-libs    #
# --------------------------------------------------------------------------- #
...
def test_single_import_reaches_all_sublibrary_keywords() -> None:
    ...
    assert len(lib._loaded_components) == 11
...
def test_full_registry_constructs_without_collision() -> None:
    # No RuntimeError over the real 11-component composition.
    lib = AgentEval()
    assert len(lib._loaded_components) == 11
